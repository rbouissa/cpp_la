...
while (std::getline(inputFile, line)) {
 ...
 modifiedString = str.substr(0, foundPos) + s2 +str.substr(foundPos + s1.length());
//std::string text_tmp;
while (foundPos != std::string::npos )
{
    ...
    foundPos = modifiedString .find(s1,foundPos+s2.length());
     modifiedString = modifiedString.substr(0, foundPos) + s2 + modifiedString.substr(foundPos + s1.length());
}
